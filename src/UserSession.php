...
class UserSession implements JsonSerializable
{
    ...
    {
        return UserSession::read_from_row(static::$DB->query("SELECT * FROM user_sessions WHERE id = ?", "i", $id)->getStatementHandler()->fetch(\PDO::FETCH_OBJ));
    }

    public static function get_by_access_token_with_user(string $access_token): static|null
    {
        return UserSession::read_from_row(
            static::$DB->query("SELECT user_sessions.id as session_id, users.id as id, username, uuid, access_token, refresh_token, server_id, expire_in, user_id, password
            FROM user_sessions
    ...
    {
        return UserSession::read_from_row(
            static::$DB->query("SELECT * FROM user_sessions WHERE refresh_token = ?", "s", $refresh_token)->getStatementHandler()->fetch(\PDO::FETCH_OBJ)
